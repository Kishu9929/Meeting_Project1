...
class OutputFormatter:
    ...
    def display_table(self, tasks: List[Dict]):
        ...
        # Improved console table formatting
        print("\n" + "="*120)
    ...
    def save_to_pdf(self, tasks: List[Dict], output_path: str = None, audio_file: str = None):
        ...
        # Total page width is 8.5 inches, minus margins (0.5 inch each side) = 7.5 inches usable
        table = Table(table_data, repeatRows=1, colWidths=[
            0.35*inch,  # # (ID)
            2.8*inch,   # Task (increased for longer descriptions - wraps text)
            0.9*inch,   # Assigned To
            1.0*inch,   # Deadline
            0.7*inch,   # Priority
            1.0*inch,   # Dependencies
            1.15*inch   # Reason
        ])
