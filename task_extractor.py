...
class TaskExtractor:
    ...
    def extract_tasks(self, text: str) -> List[Dict]:
        ...
        """
        # Keep original for name matching (case-sensitive)
        original_text = text
        ...
        # Look for task indicators - expanded patterns
        task_indicators = [
        ...
        while i < len(sentences):
            ...
            if is_task_sentence:
                # Build comprehensive context: previous + current + next sentences
                # Names are often mentioned in the previous sentence
                context_parts = []
                context_original_parts = []
                
                # Include previous sentence (often contains the name)
                if i > 0:
                ...
                if i + 1 < len(sentences):
                    ...
                    if any(word in next_sentence for word in ["by", "before", "end", "priority", "depends", "since", "understand"]):
                        ...
                        i += 1  # Skip next sentence as it's part of this task
                ...
                if task_desc:
                    # Extract assignee with enhanced context (pass full text for better name detection)
    ...
    def _extract_task_description(self, sentence: str, original_sentence: str = None) -> Optional[str]:
        ...
        # Extract task description patterns - more comprehensive
        patterns = [
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                # Remove trailing context that's not part of task description
    ...
    def _extract_assignee(self, sentence: str, original_sentence: str = None, full_text: str = None, full_text_original: str = None) -> Optional[str]:
        ...
        # Extended transcription variations (handles common Whisper errors)
        # This is custom logic based on observed transcription patterns
        name_variations = {
        ...
        # PRIORITY 1: Check for explicit name mentions in current context (highest priority)
        # This should always take precedence over role/skill matching
        for member in self.team_members:
            name = member["name"]
            name_lower = name.lower()
            
            # Check exact match in original (case-sensitive)
            if name in original_sentence:
                return name
            
            # Check exact match in lower case
            if name_lower in sentence_lower:
                return name
            
            # Check for transcription variations (handles Whisper errors)
        ...
        # PRIORITY 2: Check for patterns like "Name, we need..." or "Name you're good with..."
        # These are strong indicators of assignment
        for member in self.team_members:
            name = member["name"]
            name_lower = name.lower()
            # Pattern: "Name, we need" or "Name you're" or "Name, can you"
            patterns = [
        ...
        # PRIORITY 2.5: Check for testing/QA keywords BEFORE checking wider context
        # Testing tasks should ALWAYS go to Lata, even if other names are mentioned
        test_patterns = [
        ...
        for pattern in test_patterns:
            if re.search(pattern, sentence_lower, re.IGNORECASE):
                return "Lata"  # QA Engineer
        
        # PRIORITY 2.7: Check for API/documentation tasks - these go to Mohit (Backend)
        # This should be checked early to avoid false matches
        if re.search(r"\bapi\s+documentation\b", sentence_lower, re.IGNORECASE):
            return "Mohit"  # Backend Engineer
        
        if re.search(r"\b(?:update|write|create)\s+.*\s+documentation\b", sentence_lower, re.IGNORECASE):
            return "Mohit"  # Backend Engineer
        
        # PRIORITY 2.6: Check for design/UI/UX keywords BEFORE checking wider context
        # Design tasks should ALWAYS go to Arjun, even if other keywords are mentioned
        # BUT: Exclude API/documentation tasks (those go to backend)
        else:
            ...
            for pattern in design_keywords:
                if re.search(pattern, sentence_lower, re.IGNORECASE):
                    return "Arjun"  # UI/UX Designer
            
            # Also check for standalone design-related words
            if re.search(r"\b(?:design|onboarding|screens)\b", sentence_lower, re.IGNORECASE):
                return "Arjun"  # UI/UX Designer
            
            # Check for UI/UX but ONLY if not part of "API"
            # "API" contains "ui" but we want to exclude it
            ui_ux_match = re.search(r"\b(ui|ux)\b", sentence_lower, re.IGNORECASE)
            if ui_ux_match:
                # Check if "ui" is part of "api" - if so, skip
                match_pos = ui_ux_match.start()
                # Check 3 characters before to see if it's "api"
                if match_pos >= 3:
                    before_text = sentence_lower[max(0, match_pos-3):match_pos]
                    if "api" not in before_text:
                        return "Arjun"  # UI/UX Designer
                else:
                    # "ui" is at the start, can't be part of "api"
                    return "Arjun"  # UI/UX Designer
        
        # PRIORITY 3: Check in wider context (previous sentence might have the name)
        # This is important because names are often mentioned before the task
        # Example: "Sakshi, we need to fix the bug" - name is in previous part
        if full_text and full_text_original:
            full_text_lower = full_text.lower()
            sentence_lower_clean = sentence_lower.strip()
            
            # Find the position of current sentence in full text (try multiple methods)
            sentence_pos = -1
            # Try to find sentence position using first few words
            first_words = " ".join(sentence_lower_clean.split()[:3])
            if first_words:
                sentence_pos = full_text_lower.find(first_words)
            
            if sentence_pos >= 0:
                # Check context: 400 characters before and 100 characters after the sentence
                # Increased window to catch names mentioned earlier
                ...
                # Look for name mentions in this context window
                for member in self.team_members:
                    name = member["name"]
                    name_lower = name.lower()
                    
                    # Check exact match in original context
        ...
        # PRIORITY 4: If no explicit name, try to infer from context
        # BUT: Only if the task description doesn't contradict (e.g., "design" task shouldn't go to frontend)
        # Look for skill/role mentions, but be more careful
        # Use the full sentence context for matching
        context_lower = sentence_lower
        
        # PRIORITY 4.1: Check for specific testing/QA keywords first (most specific)
        # These should take ABSOLUTE priority - testing tasks always go to Lata
        # Testing tasks should always go to Lata (QA Engineer) regardless of other keywords
        
        # Check for explicit "write test" or "write unit test" patterns
        # These are the strongest indicators of QA/testing work
        test_patterns = [
            r"write\s+(?:unit\s+)?test",
            r"write\s+(?:unit\s+)?tests",
            r"create\s+(?:unit\s+)?test",
        ...
        # Also check full_text if available (in case context doesn't have it)
        if full_text:
            full_text_lower = full_text.lower()
            for pattern in test_patterns:
        ...
        # Check for other testing-related phrases
        testing_phrases = [
            r"unit\s+test",
            r"unit\s+tests",
            r"write\s+tests",
        ...
        # PRIORITY 4.2: Check for role mentions (more specific than skills)
        role_patterns = {
            "frontend": "Sakshi",
            "backend": "Mohit",
            "api documentation": "Mohit",  # API docs are backend
            "api doc": "Mohit",
            "documentation": "Mohit",  # Documentation is usually backend
            "ui": "Arjun",
            "ux": "Arjun",
            "design": "Arjun",
            "qa": "Lata",
        ...
        # Check role patterns but prioritize more specific matches
        ...
        for role_keyword, default_assignee in role_patterns.items():
            ...
            if re.search(pattern, context_lower, re.IGNORECASE):
                # Special case: don't match "ui" if it's part of "api"
                if role_keyword == "ui" and re.search(r"\bapi\b", context_lower, re.IGNORECASE):
                    continue
                return default_assignee
        
        # PRIORITY 4.3: Skill matching (check all skills with word boundaries)
        # Match skills in order of specificity
        skill_matches = []
        for member in self.team_members:
            for skill in member["skills"]:
                ...
                if re.search(pattern, context_lower, re.IGNORECASE):
                    skill_matches.append((len(skill_lower), member["name"]))  # Store length for priority
        
        # Return the match with longest skill name (most specific)
        if skill_matches:
            skill_matches.sort(reverse=True)  # Sort by length (longest first)
    ...
    def _extract_deadline(self, sentence: str) -> Optional[str]:
        ...
        # Check for "till/until/by [day]" patterns first (most specific)
        for day in days:
            # Patterns: "till Monday", "until Monday", "by Monday", "till next Monday"
            patterns = [
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    # Format: "Till Monday" or "Till Next Monday"
        ...
        # Check for specific deadline patterns (in order of specificity)
